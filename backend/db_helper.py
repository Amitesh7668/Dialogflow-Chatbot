...
def insert_order_item(food_items, quantity, number):
    try:
        cursor = cnx.cursor()

        # Calling the stored procedure
        cursor.callproc('insert_order_item', (food_items, quantity, number))
...
def get_order_status(number):
    cursor = cnx.cursor()

    # Executing the SQL query to fetch the order status
    query = f"SELECT status FROM order_tracking WHERE number = {number}"
    cursor.execute(query)
